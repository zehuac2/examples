...
class PriorityQueue(Generic[T, TPriority]):
  ...
  def update(
    self, element: T, old_priority: TPriority, new_priority: TPriority
  ) -> None:
  ...
  def _max_priority_child(self, index: int) -> int:
    ...
    if right < len(self._entries):
      if (
        self._priority_comparer(self._get_priority(left), self._get_priority(right))
        <= 0
      ):
        return left
      return right
  ...
  def _heapify_up(self, index: int) -> None:
    ...
    if (
      self._priority_comparer(self._get_priority(index), self._get_priority(parent)) < 0
    ):
  ...
  def _heapify_down(self, index: int) -> None:
    ...
    if (
      child != -1
      and self._priority_comparer(self._get_priority(index), self._get_priority(child))
      > 0
    ):
